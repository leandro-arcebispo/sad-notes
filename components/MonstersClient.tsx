...
import type { MonsterFull, Sprite } from "@/lib/types";
...
export default function MonstersClient({
  ...
  const cardSprites = useMemo(() => sprites.filter((s) => s.category === "monster-card"), [sprites]);
  ...
  function startEdit(m: MonsterFull) {
    setEditingId(m.id);
    setForm({ name: m.name, cardSpriteId: m.card_sprite_id });
    setMsg(null);
    setFormOpen(true);
  }
  function cancelEdit() {
  ...
  async function save() {
    ...
    const url = editingId ? `/api/monsters/${editingId}` : "/api/monsters";
              ...
              {cardSprites.length === 0 ? (
                <div className="panel">
                  <div className="center-empty">
                    Nenhum sprite “monster-card” recortado ainda.
                    <br />
            ...
            <div className="row" style={{ justifyContent: "space-between" }}>
              <div className="row">
                <button className="btn btn-accent" onClick={save} disabled={busy || !form.name.trim()}>
                  {busy ? "Salvando…" : editingId ? "Salvar alterações" : "Cadastrar monstro"}
                </button>
        ...
        {!formOpen && (
          <section className="stack" style={{ gap: 12 }}>
            ...
            {filtered.length === 0 ? (
              <div className="panel">
                <div className="center-empty">
                  {monsters.length === 0 ? "Nenhum monstro cadastrado ainda." : "Nenhum monstro encontrado."}
                </div>
              </div>
            ) : (
              <>
                <div className="treasure-grid">
                  {pageItems.map((m) => (
                    <button key={m.id} type="button" className="treasure-card" onClick={() => startEdit(m)}>
                      <div className="treasure-card-art">
                        {m.card_sprite_path ? (
                          <img className="card-art" src={assetUrl(m.card_sprite_path)} alt="" />
                        ) : (
                          <span className="muted" style={{ fontSize: 10 }}>Sem carta</span>
                        )}
                      </div>
                      <div className="pixel-label" style={{ marginTop: 8, fontSize: 13 }}>{m.name}</div>
                    </button>
                  ))}
                </div>
