    ...
    <div className="stack" style={{ gap: 22 }}>
      <div className="ornament-layout">
        ...
        <div className="controls-col">
          ...
          <div className="row">
            ...
            {editingId && (
              <button className="btn" onClick={cancelEdit} disabled={busy}>Cancelar edição</button>
            )}
      ...
      <section className="stack" style={{ gap: 12 }}>
        <h2 className="title" style={{ fontSize: 20 }}>
          Tesouros cadastrados <span className="muted" style={{ fontSize: 13 }}>({treasures.length})</span>
        </h2>
        {treasures.length === 0 ? (
          ...
          <div className="ornament-list">
            {treasures.map((t) => (
              <div key={t.id} className="ornament-row">
                <div className="ornament-thumb" title="Ícone">
                  {t.icon_sprite_path ? <img src={assetUrl(t.icon_sprite_path)} alt="" /> : <span className="muted" style={{ fontSize: 10 }}>—</span>}
                </div>
                <div className="ornament-thumb" title="Transformação">
                ...
                <div className="ornament-meta">
                  ...
                  <div className="muted" style={{ fontSize: 12 }}>{UNLOCK_MODE_LABELS[t.unlock_mode]}</div>
                </div>
                <div className="row" style={{ gap: 6 }}>
                  <button className="btn" onClick={() => startEdit(t)}>Editar</button>
                  <button className="btn btn-danger" onClick={() => del(t.id)}>Remover</button>
                </div>
              </div>
            ))}
          </div>
